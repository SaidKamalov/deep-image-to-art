...
class UGATIT_NoCycleLoss(object):
    ...
    def build_model(self):
        ...
        """ Trainer """
        self.G_optim = torch.optim.Adam(self.genA2B.parameters(), lr=self.lr,
                                        betas=(0.5, 0.999), weight_decay=self.weight_decay)

        self.D_optim = torch.optim.Adam(itertools.chain(self.disGB.parameters(), self.disLB.parameters()), lr=self.lr,
                                        betas=(0.5, 0.999), weight_decay=self.weight_decay)
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            G_loss_B = self.adv_weight * (
                    G_ad_loss_GB + G_ad_cam_loss_GB + G_ad_loss_LB + G_ad_cam_loss_LB) + self.identity_weight * G_identity_loss_B + self.cam_weight * G_cam_loss_B

            Generator_loss = G_loss_B
